...
// @OHYEAH_CUSTOM: JST timezone START
// 修正前: $default_timezone = 'Etc/UTC'; // UTC
$default_timezone = 'Asia/Tokyo'; // JST
// @OHYEAH_CUSTOM: JST timezone END
...
// @OHYEAH_CUSTOM: Japanese style timestamp START
// 修正前: $datetime_format = 'm/d/Y g:i A';
$datetime_format = 'Y/m/d H:i:s';
// @OHYEAH_CUSTOM: Japanese style timestamp END
...
if ($use_auth) {
    ...
    // @OHYEAH_CUSTOM: Save path to session for redirect before show login form START
    if (isset($_GET['p']) && !empty($_GET['p'])) {
        $_SESSION[FM_SESSION_ID]['re_path'] = $_GET['p'];
    }
    // @OHYEAH_CUSTOM: Save path to session for redirect before show login form END
...
if ((isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']]) || !FM_USE_AUTH) && isset($_POST['ajax'], $_POST['token'])) {
    ...
    if (isset($_POST['type']) && $_POST['type'] == "save") {
        ...
        // --- 追加: エンコーディングの逆変換 ---
        $origin_encoding = $_POST['origin_encoding'] ?? 'UTF-8';
        if ($origin_encoding !== 'UTF-8') {
            $writedata = mb_convert_encoding($writedata, $origin_encoding, 'UTF-8');
        }
        // ------------------------------------

        // @OHYEAH_CUSTOM: Backup before save (AJAX) START
        fm_create_backup($file_path, false); // Use copy for edits
        // @OHYEAH_CUSTOM: Backup before save (AJAX) END
            ...
            if ($chunkTotal) {
                ...
                if ($chunkIndex == $chunkTotal - 1) {
                    if (file_exists($fullPath)) {
                        // @OHYEAH_CUSTOM: Server file rename at same name file upload START
                        // 修正前: $ext_1 = $ext ? '.' . $ext : '';
                        // 修正前: $fullPathTarget = $path . '/' . basename($fullPathInput, $ext_1) . '_' . date('ymdHis') . $ext_1;
                        fm_create_backup($fullPath, true); // Use rename for uploads
                        $fullPathTarget = $fullPath; // keep uploaded file name
                        // @OHYEAH_CUSTOM: Server file rename at same name file upload END
                    } else {
                        $fullPathTarget = $fullPath;
                    }
                    // @OHYEAH_CUSTOM: Adjust server file timestamp for uploaded file START // チャンクアップロード完了後にタイムスタンプを調整
                    // 修正前: rename("{$fullPath}.part", $fullPathTarget);
                    if (rename("{$fullPath}.part", $fullPathTarget)) {
                        if (isset($_POST['last_modified'])) {
                            $mtime = intval($_POST['last_modified'] / 1000); // JSから送られたミリ秒を秒に変換
                            touch($fullPathTarget, $mtime); // 成功なら true
                            clearstatcache(true, $fullPath); // キャッシュを強制クリア
                        }

                        $response = array(
                            'status' => 'success',
                            'info' => "file upload successful (chunked)"
                        );
                    }
                    // @OHYEAH_CUSTOM: Adjust server file timestamp for uploaded file END
                }
        ...
        Dropzone.options.fileUploader = {
            ...
            init: function() {
                this.on("sending", function(file, xhr, formData) {
                    // @OHYEAH_CUSTOM: Adjust server file timestamp for uploaded file START // 元ファイルのタイムスタンプ送信追加
...
if (isset($_GET['view'])) {
    ...
    if ($online_viewer && $online_viewer !== 'false' && in_array($ext, fm_get_onlineViewer_exts())) {
        ...
        $is_text = true;
        // @OHYEAH_CUSTOM: Multi-codepage view START
        // 修正前: $content = file_get_contents($file_path);
        $raw_content = file_get_contents($file_path);
        $preview_encoding = mb_detect_encoding($raw_content, 'SJIS-win, UTF-8, eucJP-win, JIS, ASCII', true) ?: 'UTF-8';
        $content = ($preview_encoding !== 'UTF-8') ? mb_convert_encoding($raw_content, 'UTF-8', $preview_encoding) : $raw_content;
        // @MY_CUSTOM: Multi-codepage view END
   }
        ...
        <div class="col-12">
            <ul class="list-group w-50 my-3" data-bs-theme="<?php echo FM_THEME; ?>">
                ...
                if ($is_text) {
                    ...
                    // @OHYEAH_CUSTOM: Detailed charset START
                    // 修正前: echo '<li class="list-group-item"><strong>' . lng('Charset') . ':</strong> ' . ($is_utf8 ? 'utf-8' : '8 bit') . '</li>';
                    echo '<li class="list-group-item"><strong>' . lng('Charset') . ':</strong> ' . fm_enc($preview_encoding) . '</li>';
                    // @OHYEAH_CUSTOM: Detailed charset END
                }
                ?>
            </ul>
            <div class="btn-group btn-group-sm flex-wrap" role="group">
                ...
                <?php if (!FM_READONLY): ?>
                    <?php // @OHYEAH_CUSTOM: Tlanslate preview 'Delete' button START ?>
                    <!--// 修正前: <a class="fw-bold btn btn-outline-primary" title="<?php echo lng('Delete') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($file) ?>" onclick="confirmDailog(event, 1209, '<?php echo lng('Delete') . ' ' . lng('File'); ?>','<?php echo urlencode($file); ?>', this.href);"> <i class="fa fa-trash"></i> Delete</a>-->
                    ...
                    <?php // @OHYEAH_CUSTOM: Tlanslate preview 'Delete' button END ?>
                <?php endif; ?>
...
if (isset($_GET['edit']) && !FM_READONLY) {
    ...
    if (isset($_POST['savedata'])) {
        ...
        // @OHYEAH_CUSTOM: Backup before save (POST) START
        fm_create_backup($file_path, false); // Use copy for edits
        // @OHYEAH_CUSTOM: Backup before save (POST) END
        $fd = fopen($file_path, "w");
        @fwrite($fd, $writedata);
        fclose($fd);
    ...
    if (in_array($ext, fm_get_text_exts()) || substr($mime_type, 0, 4) == 'text' || in_array($mime_type, fm_get_text_mimes())) {
        $is_text = true;
        // @OHYEAH_CUSTOM: Multi-codepage edit START
        // 修正前: $content = file_get_contents($file_path);
        $raw_content = file_get_contents($file_path);
        $origin_encoding = mb_detect_encoding($raw_content, 'SJIS-win, UTF-8, eucJP-win, JIS, ASCII', true) ?: 'UTF-8';
        $content = ($origin_encoding !== 'UTF-8') ? mb_convert_encoding($raw_content, 'UTF-8', $origin_encoding) : $raw_content;
        // @OHYEAH_CUSTOM: Multi-codepage edit END
    }
...
?>
    <div class="path">
        ...
        if ($is_text && $isNormalEditor) {
            // @OHYEAH_CUSTOM: Multi-codepage edit START // Normal Editor 用隠しフィールド追加
            echo '<input type="hidden" id="origin_encoding" value="'.fm_enc($origin_encoding).'">';
            // @OHYEAH_CUSTOM: Multi-codepage edit END
           echo '<textarea class="mt-2" id="normal-editor" rows="33" cols="120" style="width: 99.5%;">' . htmlspecialchars($content) . '</textarea>';
            ...
            // @OHYEAH_CUSTOM: Multi-codepage edit START // Ace Editor 用隠しフィールド追加
            echo '<input type="hidden" id="origin_encoding" value="'.fm_enc($origin_encoding).'">';
            // @OHYEAH_CUSTOM: Multi-codepage edit END
...
// @OHYEAH_CUSTOM: Create a backup filename START
/**
 * Create a backup of a file.
 * @param string $filePath The full path to the file to be backed up.
 * @param bool $isRename Whether to rename the original file (for uploads) or copy it (for edits).
 * @return bool True on success, false on failure.
 */
function fm_create_backup($filePath, $isRename = false)
{
    ...
    $date = date("ymdHis", $mtime);

    if (substr($filename, 0, 1) === '.' && strpos($filename, '.', 1) === false) {
        $base = $filename;
        $ext = '';
    } else {
    ...
    $backupPath = $path . '/' . $base . '_' . $date . $ext;
    $counter = 0;
    while (file_exists($backupPath)) {
        $counter++;
        $backupPath = $path . '/' . $base . '_' . $date . '_' . $counter . $ext;
    }

    $success = $isRename ? @rename($filePath, $backupPath) : copy($filePath, $backupPath);

    return $success ? touch($backupPath, $mtime) : false;
}
// @OHYEAH_CUSTOM: Create a backup filename END
...
{
    // @OHYEAH_CUSTOM: Restore path from session START
    if (strpos($url, 'p=') === false) {
        if (isset($_SESSION[FM_SESSION_ID]['re_path'])) {
            // 1. 指定されたパス（ブックマーク等）があれば優先
            $re_path = $_SESSION[FM_SESSION_ID]['re_path'];
            unset($_SESSION[FM_SESSION_ID]['re_path']);
            $url .= (strpos($url, '?') === false ? '?' : '&') . 'p=' . urlencode($re_path);
        } else {
            // 2. 指定がない場合は、スクリプトがあるディレクトリを初期表示にする
            // $root_path から見た __DIR__ の相対パスを計算
            global $root_path;
            $current_dir = __DIR__;
            
            // root_path が __DIR__ の一部である場合のみ、その差分を p に入れる
            if (strpos($current_dir, $root_path) === 0) {
                $relative_path = trim(substr($current_dir, strlen($root_path)), DIRECTORY_SEPARATOR);
                if ($relative_path !== '') {
                    $url .= (strpos($url, '?') === false ? '?' : '&') . 'p=' . urlencode($relative_path);
                }
            }
        }
    }
    // @OHYEAH_CUSTOM: Restore path from session END
                ...
                if (typeof n !== 'undefined' && n !== null) {
                    if (true) {
                        var data = {
                            ...
                            type: 'save',
                            // --- 追加 ---
                            origin_encoding: $('#origin_encoding').val(),
                            // ----------
                            token: window.csrf
                        };
